...
- **users(公開プロフィール、`server/routes/publicContent.js`、Phase 3で追加)**: `GET /api/users/:userId`(`{ id, displayName, avatarUrl, bio }`を返す公開プロフィール。ユーザーが存在しなければ`404`)、`GET /api/users/:userId/public`(そのユーザーが公開した素材のみを`{ worlds, characters, scenarios, novels }`の4配列にまとめて返す。内部的に`GET /api/public/:type`相当の一覧を`ownerId`でフィルタする。ユーザーが存在しなければ`404`)。**いずれも認証不要**(publicルーターと同じ位置、`requireAuth`より前にマウント)。
