...
採用理由: (1) 保存情報を完全にコントロールできる、(2) 既存のExpress + dataStore/textStore抽象への追加が最も自然、(3) ランニングコストゼロ。

**追加依存はゼロ**とする。当初は軽量OAuthライブラリ`arctic`の利用を想定したが、arcticは内部で自前のfetchを使うため本リポジトリの`fetchImpl`注入によるテストパターンと整合しない。OAuth 2.0のトークン交換・プロファイル取得は定型的なHTTP呼び出しなので、`fetchImpl`注入の生fetchで直接実装する(実装計画で確定)。
