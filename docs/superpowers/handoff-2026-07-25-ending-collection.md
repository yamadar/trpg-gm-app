...
本機能は実装済み。データモデル・API・AI命名・UI(図鑑画面・Home/Play連携)・実績・ドキュメント同期まで完了した(設計は[specs/2026-07-25-ending-collection-design.md](specs/2026-07-25-ending-collection-design.md)、実装状況は[08-feature-ideas.md](../08-feature-ideas.md) 2章参照)。この引き継ぎ書が残していた4つの論点は次のように解決した:

1. **「セッション終了」という概念**(下記「1」参照) — 引き継ぎ時点で直前ブランチにより解決済みだった(`state.ending_reached`申告→`session.endedAt`確定の2段階)。本機能はその上に「命名して集める」層を追加しただけで、終了アクション自体は作り直していない。
2. **ダイス統計のdegree語彙(4種→6種、下記「2」参照)** — 共通指標へ丸めず、判定式アダプタの`degrees`をそのまま出し分ける方式で解決した(`src/engine/rollStats.js`)。CoC7e風の記録にだけハード成功/イクストリーム成功と正気度が現れ、他のルールセットの記録には現れない。
3. **図鑑の置き場所(下記「3」参照)** — 素材ライブラリのタブにも公開ギャラリーにも寄せず、独立のハッシュルート`#/endings`(`src/screens/EndingGallery.jsx`)をホームのボタン行から直接開く形にした。
4. **実績をやるかどうか(下記「4」参照)** — やる方向で決定した。実績はエンディング記録のコレクションから導出する純関数(`src/engine/achievements.js`、保存を持たない固定8種カタログ)として実装し、判定・保存のスコープを最小限に抑えた。
...
**2026-07-25追記(UI改善+小説化非同期化ブランチで一部解消)**: 上記の論点のうち「終了アクションの新設」「Home/Play表示」「キャンペーンとの関係」は本ブランチで実装済みになった。**GMの申告→プレイヤーの確定**の2段階: `state.ending_reached?: boolean`(GMが毎ターン`state_update.ending_reached`で結末到達を申告する一時フラグ、`src/api/prompts.js`/`src/api/turnResult.js`)がtrueかつ`session.endedAt`が未設定のとき、Play画面のログ末尾に「この物語を終える」/「まだ続ける」の確定案内カードが出る。確定すると`session.endedAt?: number`(Play画面の確定、またはキャンペーンの「次の章へ」成功時)が保存され、Home一覧・Playヘッダーの両方に「完結」バッジ(`src/components/ui/Badge.jsx`)が表示される(02-data-model.md・03-gm-logic.md・05-ui-ux.md参照)。終了の取り消しは不可(取り消しUIは無い)で、`endedAt`があってもセッションは継続可能(入力欄は塞がれない)。

**ただし「エンディングタイトル」のGM命名・図鑑化・ダイス統計・実績はいずれも未実装のまま**であり、これらが本引き継ぎ書の対象スコープとして残っている。上記で導入されたのは「セッション終了」という状態と終了アクションのみで、それをどう見せる/集計するかは別途この機能で設計する。
