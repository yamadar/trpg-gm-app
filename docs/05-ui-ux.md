...
- 公開小説の挿絵: 小説公開時に本文の挿絵マーカーと画像を公開領域へ複製する。Gallery/ユーザーページ共通の`PublicItemDetail`はマーカー位置へ画像を差し込み、マーカーが本文に残らなかった画像も末尾へ表示する。画像URLは認証不要の公開APIを使うため、未ログイン閲覧でも表示できる。
- 素材添付画像: World・Scenario・Character・Novelへ複数画像を付け、各画像の説明とトップ画像を管理できる。トップ画像は一覧/カードのサムネイル、公開詳細の先頭画像に使う。小説本文内の生成挿絵とは別機能。
...
- **集中モード**(`#/setup`・`#/play/{sessionId}`・`#/party-setup`・`#/party/{sessionId}`・Party招待): 1つのタスクを完遂する画面なのでグローバルナビもパンくずも出さず、代わりに画面側が`FocusHeader.jsx`(離脱導線+現在地+ステップ表示)を敷く。高さは`FOCUS_HEADER_HEIGHT`として公開しており、画面側の追随バーがこの直下に付く。
...
```
┌─────────────────────────────────────┐
│  [+ 新規プレイ] [+ パーティで遊ぶ]          │  ← 他画面への導線はグローバルナビ(14.0節)
│  [+ 新規キャンペーン]                      │
│                                        │
│  続きから再開                          │
│   ・セッションタイトル          続ける→│  ← 情報層
...
- Campaignタブ: 一覧選択後の詳細は「現在」「人物・勢力」「世界の動き」「章」「原典」「引き継ぎPC」「次話を作る」の7タブ。新規作成時にタイトルと原典3文書を登録し、第一話はSolo/Partyどちらで始めるか選ぶ。章タブは未精算章からAI章精算案を作り、要約・PCシート・変更項目を編集できる。Party章では全PCのシートとxpを個別編集する。変更項目は既定で未選択で、GMがチェックした項目だけ正史へ反映する。「次話を作る」は未精算章がある間は無効。GM要望から2〜3候補を生成し、一案をScenario全文へ展開、Markdownエディタで修正して通常Scenarioとして保存後、Solo/Partyどちらで始めるか選ぶ。候補の生成revisionが現在正史と違う場合はstale表示となりScenario生成不可。
...
公開一覧カードはトップ画像がある場合だけサムネイルを表示する。公開詳細`PublicItemDetail`はトップ画像をタイトル前のヒーロー画像として説明付きで表示し、残りを本文後の画像グリッドへ並べる。公開物は公開時点の画像スナップショットなので、公開元の画像編集は再公開まで反映しない。
...
- `bio`とプロフィール画像はヘッダーのアカウントメニュー(`src/components/nav/AccountMenu.jsx`)のプロフィール編集フォームから設定できる。画像はJPEG/PNG/WebPの1枚置換方式で、「画像を削除」は添付画像とOAuth由来URLを両方消す。変更は自分のユーザーページにも反映される。
...
### 14.8 Party作成・参加・Play(`PartySetup.jsx` / `PartyJoin.jsx` / `PartyPlay.jsx`、実装済み2026-08-01)

Homeの「+ パーティで遊ぶ」はParty作成へ進み、参加中PartyはSolo一覧と分けたカードでロビー/進行/停止/終了を表示する。作成画面は既存World・Scenario、2〜6体のPC、Ruleset、最大人数、行動/投票秒数、`open|character`視点、離席時方針を選び、GM用素材snapshotを共有Sessionへ複製する。Campaignの第一話/次話から来た場合はWorld、生成Scenario、`carriedPcs`、Ruleset、`campaignId`を前入力する。
