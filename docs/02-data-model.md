...
```json
{
  ...
  "gm_memory": "プレイヤーへ未公開の敵側進行・秘密状態・伏線を保持するGM専用メモ",
...
- `gm_memory`はプレイヤー向けログや`history_summary`へまだ出せない敵側進行・秘密の状態変化・未提示の伏線を保持する文字列。`state_update.gm_memory`は毎ターン更新後全文を返し、Play UIには表示せず次ターンのGM入力へ戻す。旧セッションで未定義なら空文字列扱い。
