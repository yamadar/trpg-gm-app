  ...
  schema: {
    ...
    properties: {
      ...
      choices: {
        ...
        items: { type: 'string' },
        description:
          'GMから提示する中立的な次の行動の選択肢。PC・NPC固有の語尾・口癖・方言を使わない。PCが既に知覚・把握している情報だけで書き、同じターンのnarrativeにも直近ログにも無い新事実・固有名詞を選択肢で初出させない。自由記述を促す場面では空配列',
      },
    },
  },
};
...
choicesは、上記の直近ログ・物語要約・既知フラグ・説明済み用語と、このターンのnarrativeで実際に描写した内容だけを材料に書くこと。そのどこにも無い人物・場所・物・事実を選択肢で初出させない。出したい手掛かりは先にnarrativeでPCが見聞きする形にすること。`;
}
