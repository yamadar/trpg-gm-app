...
export function createSqliteTextStore(db, { now = Date.now, coordinator } = {}) {
  ...
  return {
    async read(documentPath) {
      ...
      return execute(() => readStatement.get(documentPath)?.content ?? null);
    },
    ...
    async delete(documentPath) {
      assertPath(documentPath);
      return execute(() => deleteStatement.run(documentPath));
    },
    async deleteDir(prefix) {
      assertPath(prefix);
      return execute(() => deleteDirStatement.run(prefix, `${escapedPrefix(prefix)}/%`));
    },
  };
}
