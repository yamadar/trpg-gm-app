...
describe('Character library functions', () => {
  it('returns null for a missing character', async () => {
    expect(await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'pc', 'missing')).toBeNull();
  });
  ...
  it('saves and retrieves a pc, forcing revealed to null', async () => {
    ...
    const pc = await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'pc', 'alice');
  ...
  it('saves an npc with revealed defaulting to false when not specified', async () => {
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'npc', name: 'villain', raw: 'NPC名: 黒幕' });
    const npc = await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'npc', 'villain');
    expect(npc.revealed).toBe(false);
  });

  it('saves an npc with revealed set to true', async () => {
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'npc', name: 'villain', raw: 'x', revealed: true });
    const npc = await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'npc', 'villain');
    expect(npc.revealed).toBe(true);
  });

  it('saves an npc with revealed explicitly set to false', async () => {
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'npc', name: 'villain', raw: 'x', revealed: false });
    const npc = await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'npc', 'villain');
    expect(npc.revealed).toBe(false);
  });

  it('normalizes non-boolean truthy values to true when saving npc revealed', async () => {
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'npc', name: 'villain', raw: 'x', revealed: 1 });
    const npc = await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'npc', 'villain');
    expect(npc.revealed).toBe(true);
  });
  ...
  it('lists characters scoped to a world and kind', async () => {
    ...
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'pc', name: 'bob', raw: 'b' });
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'npc', name: 'villain', raw: 'v' });
    const pcs = await listCharacters(dataStore, 'usr_1', 'w1', 'pc');
    expect(pcs.map((c) => c.name).sort()).toEqual(['alice', 'bob']);
    const npcs = await listCharacters(dataStore, 'usr_1', 'w1', 'npc');
  ...
  it('scopes listing to the given world (does not leak characters from other worlds)', async () => {
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'pc', name: 'alice', raw: 'a' });
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w2', kind: 'pc', name: 'carol', raw: 'c' });
    const pcs = await listCharacters(dataStore, 'usr_1', 'w1', 'pc');
  ...
  it('deletes a character', async () => {
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'pc', name: 'alice', raw: 'a' });
    await deleteCharacter(dataStore, textStore, 'usr_1', 'w1', 'pc', 'alice');
    expect(await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'pc', 'alice')).toBeNull();
  });
...
describe('saveCharacterParsed', () => {
  it('returns null when the character does not exist', async () => {
    const result = await saveCharacterParsed(dataStore, 'usr_1', 'w1', 'pc', 'missing', {
  ...
  it('updates parsed and parsedHash without touching raw text', async () => {
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'pc', name: 'alice', raw: '原文' });

    const updated = await saveCharacterParsed(dataStore, 'usr_1', 'w1', 'pc', 'alice', {
    ...
    const character = await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'pc', 'alice');
  ...
  it('preserves other meta fields (revealed) when updating parsed', async () => {
    await saveCharacter(dataStore, textStore, 'usr_1', { worldId: 'w1', kind: 'npc', name: 'villain', raw: 'x', revealed: true });
    await saveCharacterParsed(dataStore, 'usr_1', 'w1', 'npc', 'villain', {
      parsed: { goal: 'a', bonds: 'b' },
      parsedHash: 'h',
    });
    const character = await getCharacter(dataStore, textStore, 'usr_1', 'w1', 'npc', 'villain');
