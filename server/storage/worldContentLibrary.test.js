...
describe('Region functions', () => {
  it('returns null for a missing region', async () => {
    expect(await getRegion(textStore, 'usr_1', 'w1', 'missing')).toBeNull();
  });

  it('saves and retrieves a region', async () => {
    await saveRegion(textStore, 'usr_1', 'w1', 'waterdeep', '地域の詳細');
    expect(await getRegion(textStore, 'usr_1', 'w1', 'waterdeep')).toBe('地域の詳細');
  });

  it('lists region ids scoped to a world', async () => {
    await saveRegion(textStore, 'usr_1', 'w1', 'waterdeep', 'a');
    await saveRegion(textStore, 'usr_1', 'w1', 'baldurs-gate', 'b');
    await saveRegion(textStore, 'usr_1', 'w2', 'other-world-region', 'c');
    const regions = await listRegions(textStore, 'usr_1', 'w1');
    expect(regions.sort()).toEqual(['baldurs-gate', 'waterdeep']);
  });

  it('returns an empty list when a world has no regions', async () => {
    expect(await listRegions(textStore, 'usr_1', 'w1')).toEqual([]);
  });

  it('deletes a region', async () => {
    await saveRegion(textStore, 'usr_1', 'w1', 'waterdeep', 'a');
    await deleteRegion(textStore, 'usr_1', 'w1', 'waterdeep');
    expect(await getRegion(textStore, 'usr_1', 'w1', 'waterdeep')).toBeNull();
  });
});

describe('Category functions', () => {
  it('returns null for a missing category', async () => {
    expect(await getCategory(textStore, 'usr_1', 'w1', 'missing')).toBeNull();
  });

  it('saves and retrieves a category', async () => {
    await saveCategory(textStore, 'usr_1', 'w1', 'magic-system', 'カテゴリの詳細');
    expect(await getCategory(textStore, 'usr_1', 'w1', 'magic-system')).toBe('カテゴリの詳細');
  });

  it('lists category ids scoped to a world', async () => {
    await saveCategory(textStore, 'usr_1', 'w1', 'magic-system', 'a');
    await saveCategory(textStore, 'usr_1', 'w1', 'history', 'b');
    const categories = await listCategories(textStore, 'usr_1', 'w1');
    expect(categories.sort()).toEqual(['history', 'magic-system']);
  });

  it('deletes a category', async () => {
    await saveCategory(textStore, 'usr_1', 'w1', 'magic-system', 'a');
    await deleteCategory(textStore, 'usr_1', 'w1', 'magic-system');
    expect(await getCategory(textStore, 'usr_1', 'w1', 'magic-system')).toBeNull();
  });
});
